...
def separate_image(raw_file_name):
    ...
    if len(raw_name) != 4:
        ...
        ##############################################################################
    # Binarize image data
    raw_image=cv2.imread(raw_file_name)
    print("raw_image:{} {}".format(raw_image.shape,raw_image.dtype))
    ...
    for k, col in zip(range(n_clusters), colors):
        ...
        blank_item = np.zeros(shape=(90, 250, 3), dtype="uint8")
        # print(my_members)
...
def separate_image1(raw_file_name,raw_image=None):
    file_name = os.path.basename(raw_file_name)
    raw_name, ext_name = os.path.splitext(file_name)
    if len(raw_name) != 4:
        print('wrong raw_name:', raw_name)
    ...
    im = gray_image
    # print(h,w)
    X = [(x, y) for x in range(h) for y in range(w) if im[x][y] and x > 0 and y > 0]
    X = np.array(X)
    n_clusters = 4
    # print("x",type(X),X.shape,X.ndim)
